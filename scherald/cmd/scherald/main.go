// scherald — Scheduled-audit Herald per spec V3 §18 + §43 (Wave 2 r1).
//
// Single-binary CLI for the Scheduled-audit Herald flavor (periodic
// Status.md sweep + compliance digest). CLI-only — no HTTP serve plane
// (DefaultPort=0 per branding); only §43 stubs are registered.
package main
...
func main() {
	...
	branding := commons.DefaultBranding("sc", version)
