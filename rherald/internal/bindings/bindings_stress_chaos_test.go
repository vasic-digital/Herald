...
// HRD-022 §11.4.85 stress + chaos for the rherald release bindings pipeline.
//
...
// release-event load and under emit-fault injection — the resilience-layer
...
// Every Subject is a fabricated release-result string — NO real tag/push/git/
// install. The release detectors DETECT/classify only; they never tag or push.
...
func bindingsSurface(t *testing.T) (*stresschaos.SurfaceDir, bool) {
	...
	if runID == "" {
		runID = stresschaos.NewRunID("hrd022")
	}
...
// faultEmitter wraps a real emitter but makes ReleaseGateBlocked fail — the
...
func (f faultEmitter) ReleaseGateBlocked(ctx context.Context, e constitution.ReleaseEvent) error {
	return f.err
}

// TestBindings_Stress_ConcurrentReleaseBlocks drives N=8 workers × M=150
// EvaluateSubject calls (1200 total) for DISTINCT failing pre-tag-retest gates
// (§11.4.40, critical, enforce) through ONE shared Pipeline. Asserts: zero
// errors, every call FAILs (real release-gate block), and the bus saw exactly
// the expected number of .release.gate.blocked emits (no lost events under
// contention). Under -race this is the data-race proof for the whole
// evaluate→record→emit→audit stack.
func TestBindings_Stress_ConcurrentReleaseBlocks(t *testing.T) {
	...
	tenant := uuid.New()
	if err := la.Set(ctx, tenant, "§11.4.40", constitution.ModeEnforce, "stress"); err != nil {
	...
	sum := stresschaos.RunLoad(workers, iters, func(workerID, iter int) error {
		// Distinct release tag per (worker,iter) so each is a FirstSeen transition
		// → each MUST emit exactly once (no transitions-only suppression). Every
		// retest is "skipped" → a §11.4.40 release-gate block.
		subj := constitution.Subject{Kind: bindings.SubjectRetestGate, ID: fmt.Sprintf("v%d.%d.0|retest=skipped", workerID, iter)}
		...
		if out.Decision == constitution.DecisionFail {
			...
			return fmt.Errorf("retest %s got %v, want fail", subj.ID, out.Decision)
		}
		if !out.Emitted {
			return fmt.Errorf("enforce release-block did not emit: %s", subj.ID)
		}
		return nil
	})
	...
	if sum.Errors != 0 {
		t.Fatalf("concurrent release-blocks reported %d errors (want 0)", sum.Errors)
	}
	...
	// The bus MUST have published exactly `total` .release.gate.blocked events —
	// no event lost under contention.
	published := bus.Metrics().PublishedByType[constitution.EventNamespace+"."+constitution.ClassReleaseGateBlocked]
	if published != total {
		t.Fatalf("bus published %d .release.gate.blocked events, want %d (lost events under load)", published, total)
	}
	// Audit rows MUST equal total too.
	audit, err := au.ListAudit(ctx, tenant, constitution.AuditQuery{RuleID: "§11.4.40"})
	if err != nil {
	...
	report := fmt.Sprintf(
		"surface=bindings scenario=stress_concurrent_release_blocks rule=§11.4.40 mode=enforce severity=critical\n"+
			"workers=%d iters_per_worker=%d total=%d\n"+
			"fails=%d non_fail=%d errors=%d\n"+
			"bus_published_release_gate_blocked=%d audit_rows=%d\n"+
			"no_lost_events_under_load=%d\n"+
			"p50_ms=%.4f p95_ms=%.4f p99_ms=%.4f max_ms=%.4f tput_per_sec=%.1f\n"+
			"race_detector=clean no_real_tag_push_git=true\n",
		workers, iters, total,
		atomic.LoadInt64(&fails), atomic.LoadInt64(&nonFail), sum.Errors,
		published, len(audit),
		boolToInt022(published == total && int64(len(audit)) == total),
		sum.Latency.P50MS, sum.Latency.P95MS, sum.Latency.P99MS, sum.Latency.MaxMS, sum.ThroughputPS)
	if _, err := sd.WriteFile("concurrent_release_blocks.log", report); err != nil {
		t.Fatalf("write report: %v", err)
	}
	writeBindingsSummary(t, sd, sum, persistent)
	t.Logf("rherald bindings stress[concurrent]: %d evals, 0 errors, %d emits, %d audit rows, p99=%.3fms (persistent=%v dir=%s)",
...
// ReleaseGateBlocked always errors) and floods the pipeline with blocked
// release gates. The §107 fail-loud contract: EVERY enforce-mode release-block
...
func TestBindings_Chaos_EmitFaultSurfaces(t *testing.T) {
	...
	defer bus.Close()
	realEm, err := constitution.NewEmitter(bus, constitution.EmitterConfig{Source: "digital.vasic.herald/rherald"})
	if err != nil {
		t.Fatalf("NewEmitter: %v", err)
	}
	...
	tenant := uuid.New()
	if err := la.Set(ctx, tenant, "§11.4.40", constitution.ModeEnforce, "chaos"); err != nil {
		t.Fatalf("ladder Set: %v", err)
	}
	...
	sum := stresschaos.RunLoad(workers, iters, func(workerID, iter int) error {
		subj := constitution.Subject{Kind: bindings.SubjectRetestGate, ID: fmt.Sprintf("v%d.%d.0|retest=skipped", workerID, iter)}
		_, err := p.EvaluateSubject(ctx, "§11.4.40", tenant, subj)
		if err != nil {
	...
	faultLog := fmt.Sprintf(
		"surface=bindings scenario=chaos_emit_fault emitter=faultEmitter(ReleaseGateBlocked→err)\n"+
			"contract: enforce release-block + bus drop → EvaluateSubject errors, NEVER silent success\n"+
		...
		boolToInt022(atomic.LoadInt64(&swallowed) == 0 && atomic.LoadInt64(&surfaced) == total),
		sum.Latency.P99MS, sum.Count)
	if _, err := sd.WriteFile("emit_fault_fail_loud.log", faultLog); err != nil {
		t.Fatalf("write fault log: %v", err)
	}
	t.Logf("rherald bindings chaos[emit-fault]: %d evals → %d surfaced fault, 0 swallowed", total, atomic.LoadInt64(&surfaced))
}

func boolToInt022(b bool) int {
	if b {
		return 1
	}
	return 0
...
func writeBindingsSummary(t *testing.T, sd *stresschaos.SurfaceDir, healthy stresschaos.LoadSummary, persistent bool) {
	...
	md := fmt.Sprintf(`# Stress + Chaos — rherald release constitution bindings (HRD-022)
...
|---|---|---|---|
| stress_concurrent_release_blocks | PASS | concurrent_release_blocks.log, latency.json | %d evals, 0 errors, every .release.gate.blocked emit + audit accounted (no lost events), p50=%.3fms p95=%.3fms p99=%.3fms tput=%.0f/s |
