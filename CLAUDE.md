...
- **Container stack:** Postgres (main DB) + Redis (in-memory) bundled via the `containers` submodule (`https://github.com/vasic-digital/containers`). All container names are `herald-`-prefixed (`herald-postgres`, `herald-redis`, `herald-otel`, `herald-pherald`); all host ports are in the **`24XXX`** reserved range — Postgres `24100`, Redis `24200`, OTel `24417`/`24418`, and the per-flavor serve ports from `commons/branding.go` (`pherald` 24791, `sherald` 24793, …) — to avoid collisions. (An earlier "70XXX" framing is stale; the committed `quickstart/docker-compose.quickstart.yml` + `commons/branding.go` use 24XXX.)
