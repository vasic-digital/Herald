...
// HRD-021 §11.4.85 stress + chaos for the bherald CI/test bindings pipeline.
//
...
// gate-result load and under emit-fault injection — the resilience-layer
...
// Every Subject is a fabricated gate-result string — NO real build/test runs.
...
func bindingsSurface(t *testing.T) (*stresschaos.SurfaceDir, bool) {
	...
	if runID == "" {
		runID = stresschaos.NewRunID("hrd021")
	}
...
// faultEmitter wraps a real emitter but makes GateFailed fail — the hermetic
// analogue of an eventbus drop mid-emit. The Pipeline MUST surface that as an
// error (no silent swallow → no §107 fail-bluff at the distribution layer).
type faultEmitter struct {
	constitution.EventEmitter
	err error
}

func (f faultEmitter) GateFailed(ctx context.Context, e constitution.GateEvent) error {
	return f.err
}

// TestBindings_Stress_ConcurrentGateFailures drives N=8 workers × M=150
// EvaluateSubject calls (1200 total) for DISTINCT failing gate-results of the §1
// gate-result rule (enforce mode) through ONE shared Pipeline. Asserts: zero
// errors, every call FAILs (real gate failure), and the bus saw exactly the
// expected number of .gate.failed emits (no lost events under contention).
// Under -race this is the data-race proof for the whole evaluate→record→emit→
// audit stack.
func TestBindings_Stress_ConcurrentGateFailures(t *testing.T) {
	...
	tenant := uuid.New()
	if err := la.Set(ctx, tenant, "§1", constitution.ModeEnforce, "stress"); err != nil {
	...
	sum := stresschaos.RunLoad(workers, iters, func(workerID, iter int) error {
		// Distinct gate per (worker,iter) so each is a FirstSeen transition → each
		// MUST emit exactly once (no transitions-only suppression).
		subj := constitution.Subject{Kind: bindings.SubjectGateResult, ID: fmt.Sprintf("gate_%d_%d|outcome=fail", workerID, iter)}
		...
		if out.Decision == constitution.DecisionFail {
			...
			return fmt.Errorf("gate %s got %v, want fail", subj.ID, out.Decision)
		}
		if !out.Emitted {
			return fmt.Errorf("enforce gate-failure did not emit: %s", subj.ID)
		}
		return nil
	})
	...
	if sum.Errors != 0 {
		t.Fatalf("concurrent gate-failures reported %d errors (want 0)", sum.Errors)
	}
	...
	// The bus MUST have published exactly `total` .gate.failed events — no event
	// lost under contention.
	published := bus.Metrics().PublishedByType[constitution.EventNamespace+"."+constitution.ClassGateFailed]
	if published != total {
		t.Fatalf("bus published %d .gate.failed events, want %d (lost events under load)", published, total)
	}
	...
	report := fmt.Sprintf(
		"surface=bindings scenario=stress_concurrent_gate_failures rule=§1 mode=enforce\n"+
			"workers=%d iters_per_worker=%d total=%d\n"+
			"fails=%d non_fail=%d errors=%d\n"+
			"bus_published_gate_failed=%d audit_rows=%d\n"+
		...
		published, len(audit),
		boolToInt021(published == total && int64(len(audit)) == total),
		sum.Latency.P50MS, sum.Latency.P95MS, sum.Latency.P99MS, sum.Latency.MaxMS, sum.ThroughputPS)
	if _, err := sd.WriteFile("concurrent_gate_failures.log", report); err != nil {
		t.Fatalf("write report: %v", err)
	}
	writeBindingsSummary(t, sd, sum, persistent)
	t.Logf("bherald bindings stress[concurrent]: %d evals, 0 errors, %d emits, %d audit rows, p99=%.3fms (persistent=%v dir=%s)",
...
// GateFailed always errors) and floods the pipeline with failing gates. The
// §107 fail-loud contract: EVERY enforce-mode gate-failure MUST surface the
// emit error from EvaluateSubject — NEVER a silently-swallowed success. A
// pipeline that returned RunOutcome{Emitted:true} while the wire emit failed
// would be a distribution-layer PASS-bluff; this test makes that impossible.
func TestBindings_Chaos_EmitFaultSurfaces(t *testing.T) {
	...
	defer bus.Close()
	realEm, err := constitution.NewEmitter(bus, constitution.EmitterConfig{Source: "digital.vasic.herald/bherald"})
	if err != nil {
		t.Fatalf("NewEmitter: %v", err)
	}
	...
	tenant := uuid.New()
	if err := la.Set(ctx, tenant, "§1", constitution.ModeEnforce, "chaos"); err != nil {
		t.Fatalf("ladder Set: %v", err)
	}
	...
	sum := stresschaos.RunLoad(workers, iters, func(workerID, iter int) error {
		subj := constitution.Subject{Kind: bindings.SubjectGateResult, ID: fmt.Sprintf("gate_%d_%d|outcome=fail", workerID, iter)}
		_, err := p.EvaluateSubject(ctx, "§1", tenant, subj)
		if err != nil {
	...
	faultLog := fmt.Sprintf(
		"surface=bindings scenario=chaos_emit_fault emitter=faultEmitter(GateFailed→err)\n"+
			"contract: enforce gate-failure + bus drop → EvaluateSubject errors, NEVER silent success\n"+
		...
		boolToInt021(atomic.LoadInt64(&swallowed) == 0 && atomic.LoadInt64(&surfaced) == total),
		sum.Latency.P99MS, sum.Count)
	if _, err := sd.WriteFile("emit_fault_fail_loud.log", faultLog); err != nil {
		t.Fatalf("write fault log: %v", err)
	}
	t.Logf("bherald bindings chaos[emit-fault]: %d evals → %d surfaced fault, 0 swallowed", total, atomic.LoadInt64(&surfaced))
}

func boolToInt021(b bool) int {
	if b {
		return 1
	}
	return 0
...
func writeBindingsSummary(t *testing.T, sd *stresschaos.SurfaceDir, healthy stresschaos.LoadSummary, persistent bool) {
	...
	md := fmt.Sprintf(`# Stress + Chaos — bherald CI/test constitution bindings (HRD-021)
...
|---|---|---|---|
| stress_concurrent_gate_failures | PASS | concurrent_gate_failures.log, latency.json | %d evals, 0 errors, every .gate.failed emit + audit accounted (no lost events), p50=%.3fms p95=%.3fms p99=%.3fms tput=%.0f/s |
