...
| Supersedes | `docs/specs/mvp/archive/specification.V3.md` (V3 r13, 2026-05-28). V4 is a strict **superset**: it preserves every V3 section and weaves in two new mandatory features — **participant identity + workable-item attribution + notification @-tagging** (§7.6, §8.4, §32.6.A, §34.7) and **natural-language intent recognition + clarify-and-tag** (§32.6.B, §33.8, §34.8). Normative detail lives in the two design contracts `docs/design/PARTICIPANT_ATTRIBUTION.md` + `docs/design/INTENT_RECOGNITION.md` and the inherited constitution anchors §11.4.104 + §11.4.105. |
| What changed from V3 | **(1) Participant identity (Subscribers/Users):** §7.6 formalizes the logical `subscribers` + per-channel `subscriber_aliases.username` model — one person carries a DIFFERENT `@username` per messenger; the `HERALD_<CHANNEL>_OPERATOR_USERNAME` operator designation (env-var, not a DB flag); and the `Claude` system-agent sentinel handle. **(2) Workable-item attribution:** §8.4 adds `created_by` + `assigned_to` (canonical-handle strings) to every workable item with the attribution rules (CLI-prompt→Operator / system-detected→`Claude` / received-message→@sender; `assigned_to` default = Operator). **(3) Notification @-tagging matrix:** §34.7 adds the outbound tagging rules (tag `assigned_to` if human≠Operator; tag `created_by` if human≠Operator≠`Claude`; Operator+`Claude` never tagged; skip if the participant has no alias on the target channel). **(4) Natural-language intent recognition:** §32.6.B + §33.8 + §34.8 add the three-tier resolution (Tier-1 deterministic `CommandRecognizer`, Tier-2 LLM intent inference via the dispatch envelope, Tier-3 clarify-and-tag), the recognized command set, and the new `clarify` `<<<HERALD-REPLY>>>` action — "no command syntax required / never guess / never ignore". As-built status per feature is annotated inline (LIVE vs contract-being-implemented). Full V3→V4 changelog: §29.3. |
...
- [§7. Subscriber model](#7-subscriber-model)
  ...
  - [7.6 Participant identity (logical Subscriber/User + per-channel @username) — V4](#76-participant-identity-logical-subscriberuser--per-channel-username--v4)
- [§8. Workable-item naming prefix](#8-workable-item-naming-prefix)
  ...
  - [8.4 Workable-item attribution (`created_by` + `assigned_to`) — V4](#84-workable-item-attribution-created_by--assigned_to--v4)
...
- [§29. Changelog](#29-changelog)
  ...
  - [29.3 V3 → V4 changes (2026-05-31)](#293-v3-v4-changes-2026-05-31)
...
- [§32. Inbound processing pipeline](#32-inbound-processing-pipeline)
  - [32.6.A Inbound attribution — setting `created_by` / `assigned_to` — V4](#326a-inbound-attribution--setting-created_by--assigned_to--v4)
  - [32.6.B Natural-language intent recognition (three-tier resolution) — V4](#326b-natural-language-intent-recognition-three-tier-resolution--v4)
- [§33. LLM / agent dispatch](#33-llm-agent-dispatch)
  - [33.8 Tier-2 envelope intent instruction + the `clarify` action — V4](#338-tier-2-envelope-intent-instruction--the-clarify-action--v4)
- [§34. Reply protocol (queued → processing → result)](#34-reply-protocol-queued-processing-result)
  - [34.7 Notification @-tagging matrix (outbound participant mentions) — V4](#347-notification--tagging-matrix-outbound-participant-mentions--v4)
  - [34.8 Tier-3 clarify reply (tag-and-ask fallback) — V4](#348-tier-3-clarify-reply-tag-and-ask-fallback--v4)
...
```sql
-- migration 000015 (V4) — additive to the §7.1 subscriber_aliases schema:
ALTER TABLE subscriber_aliases ADD COLUMN username TEXT;   -- per-channel @handle for tagging
CREATE INDEX subscriber_aliases_username_idx ON subscriber_aliases (channel, username)
...
- a human's **canonical handle** — defaults to their Telegram `@username` (Telegram is the primary messenger) but is messenger-neutral; per-channel `@username`s are resolved through `subscriber_aliases.username`.
...
type Participant struct {
    ...
    DisplayName string
    Kind        string            // "human" | "agent" | "service" (§7.5)
...
type IdentityResolver interface {
    ...
    // Outbound: the @username for a canonical handle on a target channel
    // (ok=false if the participant has no alias there — cannot tag someone
...
`MemoryResolver` is the concrete in-memory resolver built from a participant roster + the operator handle; unknown senders resolve to their raw normalized `@username` so first-contact users stay attributable. `OperatorHandleFromEnv(channel)` reads `HERALD_<CHANNEL>_OPERATOR_USERNAME`.
...
### 8.4 Workable-item attribution (`created_by` + `assigned_to`) — V4

> **New in V4** (operator mandate 2026-05-31). Normative contract: `docs/design/PARTICIPANT_ATTRIBUTION.md` §2 + §4. Constitution authority: inherited §11.4.104. **As-built:** LIVE — `commons_workable` `Item.CreatedBy`/`Item.AssignedTo` (+ SQLite columns + change-feed); inbound wiring `pherald/internal/inbound/attribution.go`; the workable-items SSoT tool (`constitution/scripts/workable-items/`) adds the same two columns.
...
| Origin of the item | `created_by` |
|---|---|
| Opened via the **Claude Code CLI prompt** (operator-driven) | `OperatorHandle()` — the operator's canonical handle |
| Opened by **System/Claude** detecting an issue/task/improvement/missing-feature | `Claude` (`commons.SystemAgentHandle`) |
| Received **through Herald** (a subscriber message opens/updates an item) | the sender's canonical handle, resolved via `IdentityResolver.ResolveSender(channel, channelUserID, @username)` |
...
- **`commons_workable`** (Herald's mirror store): `Item` gains `CreatedBy string` + `AssignedTo string`; the SQLite `items` table + Go migration add the two columns; the change-feed `Diff` emits `item.field.changed` for `created_by`/`assigned_to` changes.
...
- V4 is a strict **superset** — every V3 section is preserved; V4 only **adds**. Two operator-mandated features (2026-05-31) are woven into the existing structure:
  ...
  **Feature 2 — Natural-language intent recognition + clarify-and-tag** (normative: `docs/design/INTENT_RECOGNITION.md`; constitution: inherited §11.4.105):
  ...
  - **§33.8 Tier-2 envelope instruction + the `clarify` action** — the additive INTENT RECOGNITION envelope block; the new `<<<HERALD-REPLY>>>` `action: "clarify"` + `Question` field (action set now seven values).
...
|---|---|---|
| Telegram | Bot API `getUpdates` long-poll (timeout 25 s) — **LIVE (Wave 6 / Wave 7)** | continuous, but a 30 s safety-net timer also fires `getUpdates` if the long-poll thread stalled |
| Slack | Socket Mode (WebSocket) OR Events API webhook — **LIVE (Wave 7 T6 — Socket Mode)** | continuous on Socket Mode; 30 s timer as keepalive ping |
...
### 32.6.A Inbound attribution — setting `created_by` / `assigned_to` — V4

> **New in V4** (operator mandate 2026-05-31). Normative contract: `docs/design/PARTICIPANT_ATTRIBUTION.md` §2 + §5. Constitution authority: inherited §11.4.104. **As-built:** LIVE — `pherald/internal/inbound/attribution.go` + `dispatcher.go` + `item_mutator.go`.
...
> **New in V4** (operator mandate 2026-05-31). Normative contract: `docs/design/INTENT_RECOGNITION.md` §1–§2 + §5. Constitution authority: inherited §11.4.105. **As-built:** Tier-1 `CommandRecognizer` + Tier-2 envelope instruction + Tier-3 `clarify` action are implemented (`pherald/internal/inbound/command_recognizer.go`, `commons_messaging/dispatch/claude_code` envelope, `reply.go` `clarify`); the Tier-1 recognizer + Tier-3 clarify handler are landing in a parallel implementation stream and are described here per the contract (the normal role of a spec).
...
TIER 1  command recognition  — a deterministic CommandRecognizer maps clear
                               ...
                               WITHOUT an LLM round-trip (fast-path; no prefix
                               required). Confident match → that action.
TIER 2  intent inference     — when no command matches, the Claude Code dispatch
                               (§33) infers the intent and returns a
                               <<<HERALD-REPLY>>> action. The envelope (§33.8)
                               INSTRUCTS the LLM to recognize Herald's command
                               set, map plain language to the right action, and
                               NEVER guess.
TIER 3  clarify (fallback)   — when neither a command nor a confident intent can
                               be determined, the action is "clarify": the System
                               replies to the original message, tags the sender
                               (@username, via the §7.6 IdentityResolver), and
                               asks a precise clarifying question. No guessing,
                               no silent drop (§34.8).
```
...
| "open a bug: `<title>`", "create a task: `<title>`", "new feature request: `<title>`" | `issue.open` | type+title (created_by=sender, §32.6.A) |
...
> **New in V4** (operator mandate 2026-05-31). Normative contract: `docs/design/INTENT_RECOGNITION.md` §3 + §4. Constitution authority: inherited §11.4.105. **As-built:** `FormatEnvelope` gains the intent-instruction block; `<<<HERALD-REPLY>>>` gains `action: "clarify"` + `Question` field (`pherald/internal/inbound/reply.go`).
...
# UsernameFor(handle, channel) — SKIP if the participant has no alias on that channel.
```
...
`commons.MentionsFor(createdBy, assignedTo, operatorHandle, channel, r IdentityResolver) []string` returns the canonical handles to mention (NOT the resolved `@usernames`), in stable de-duplicated order, having dropped `Claude`, the operator, and any handle with no alias on `channel`. Each channel adapter renders the resolved `@username`s in its own mention syntax — Telegram prepends/appends a `cc: @username` line (`tgram.PrependMentions`); Slack `<@U…>` and other adapters render their native mention form (future). §107 anti-bluff: the outbound E2E asserts the captured dispatched body contains exactly the expected `@username`s, with a NEGATIVE case proving the Operator is never self-tagged; a paired mutation that removes the operator-skip makes the E2E FAIL.

### 34.8 Tier-3 clarify reply (tag-and-ask fallback) — V4

> **New in V4** (operator mandate 2026-05-31). Normative contract: `docs/design/INTENT_RECOGNITION.md` §3 + §5. Constitution authority: inherited §11.4.105. **As-built:** the `clarify` handler is landing in the parallel intent-recognition stream; described here per the contract.
...
| Wave 7 multi-channel (2026-05-28) — generic messenger framework | `pherald` | `pherald listen` (multi-channel mode) | (no REST; long-running CLI daemon) | `HERALD_CHANNELS=tgram,slack` (default `tgram`) spins up one `Subscribe` goroutine per enabled channel, all fanning into the SAME `inbound.Dispatcher`. Per-channel namespaced env: `HERALD_TGRAM_*` (Wave 6) + `HERALD_SLACK_BOT_TOKEN` / `HERALD_SLACK_APP_TOKEN` / `HERALD_SLACK_CHANNEL_ID`. The generalized self-filter `channels.IsSelfEcho` drops bot-own echoes uniformly across IdentityUsername (tgram) and IdentityUserID (slack). Attachments isolate under `~/.herald/inbox/<channel>/<sha256>.<ext>`. Slack adapter is Socket Mode (parity with tgram long-poll — no public webhook required). | HRD-110..HRD-115 |
...
| V3 | `docs/specs/mvp/archive/specification.V3.md` | 13 | superseded | ~5000 | Historical (operator-product layer + nine refined flavors + Waves 2–7) |
| V4 | `docs/specs/mvp/specification.V4.md` | 1 | active | ~5200 | **Current** — adds participant identity/attribution/@-tagging (§7.6/§8.4/§34.7) + natural-language intent recognition (§32.6.B/§33.8/§34.8) |
