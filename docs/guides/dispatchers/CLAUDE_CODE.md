...
| Herald spec V3 §33 + §33.2 + §33.3 | `docs/specs/mvp/specification.V4.md` | The §33.2 session-resolution algorithm (anchor-file at `<workdir>/.herald/claude-code/sessions/<project>.session`); the §33.3 envelope + `<<<HERALD-REPLY>>>` marker contract; the `Outcome` + `Summary` fields the §107 anti-bluff guard asserts non-empty. |
