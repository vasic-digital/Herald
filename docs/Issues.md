...
| Status | active |
| Status summary | **r14 captures Wave 4b (TOON `application/toon` content negotiation; tag v0.3.0). No new HRDs migrated — substrate-level upgrade.** Every `/v1/*` business route on every serving flavor now honors `Accept: application/toon` → real TOON-encoded body (via `digital.vasic.toon` + `toon-format/toon-go` upstream, real wire-up replacing the prior sentinel-error scaffold from the §107 PASS-bluff revert). JSON remains the honest fallback. `POST /v1/events` accepts both TOON and JSON request bodies. TOONMiddleware auto-wired into `cli.RunServe` — every flavor inherits. 7 new e2e invariants E56-E62 (E62 SKIP-with-reason — encoder-failure fallback unit-tested in `commons/cli/toon_test.go`). New `tests/test_wave4b_mutation_meta.sh` (5/5 PASS — M1 strip TOONMiddleware → E57 FAIL; M2 swap `toon.Marshal` → `json.Marshal` (recurrence of the 2026-05-17 PASS-bluff) → E60 FAIL; M3 no-op transcodeRequestBody → pherald unit test FAIL; working-tree quiescence check (lesson from 72e81ab); post-flight green). Tag **v0.3.0**. Prior r13 captured Wave 4a (HTTP/3 + Brotli + Alt-Svc + TLS 1.3). r12 closed HRD-016 (Wave 3b pherald Runner live). r11 closed HRD-011 (Telegram live, message_id=5). r10 renumbered HRD-093 → HRD-099. r9: Wave 3a close-out (HRD-028 + HRD-098). |
