...
| Status | active |
| Status summary | **r14 captures Wave 4a (HTTP/3 + Brotli + Alt-Svc + TLS 1.3 transport substrate). v0.2.0 tag issued.** Every serving flavor (pherald + cherald + sherald + iherald) now binds dual TCP/HTTP-2 + UDP/HTTP-3 listeners on the same port. Dev TLS cert auto-generated at `~/.herald/dev-{cert,key}.pem` (ECDSA P-256, SAN localhost/127.0.0.1/::1, 365-day validity, chmod 600 key); prod requires `--tls-cert`/`--tls-key` flags or `HERALD_TLS_CERT_PATH`/`HERALD_TLS_KEY_PATH` env. Brotli q6 default (`HERALD_BROTLI_LEVEL` env override) compresses responses ≥256B. Alt-Svc header `h3="<port>"; ma=2592000` advertises HTTP/3 to TCP clients. `HERALD_DISABLE_HTTP3=1` escape hatch for UDP-blocked envs. New `commons_tls/` module (workspace 14 → 15). 7 new e2e invariants E49-E55 (E55 SKIP-with-reason for tcpdump root requirement). `tests/test_wave4_mutation_meta.sh` (4/4 PASS — M1 strip H3 → E50 FAIL; M2 strip Brotli → E53 FAIL; M3 downgrade TLS → E54 FAIL; post-flight). **Critical security fix (d5bd360) restoring commons_auth/middleware.go from JWT-bypass mutation residue that the prior FIX-bg subagent's 72e81ab accidentally captured mid-flight from a concurrent mutation gate.** Lesson — subagent commits must audit working-tree for mutation markers before staging. Prior r13 closed HRD-016 (Wave 3b pherald Runner live). r12 closed HRD-011 (Telegram live). r10-r11: Wave 3a substrate. |
...
| Continuation | **Wave 4b next**: TOON (Token-Oriented Object Notation) adoption as preferred Content-Type with JSON fallback. Wave 4b design lives at `docs/superpowers/specs/2026-05-22-http3-brotli-toon-design.md` §5. After Wave 4b: queued QA-bot workstream — `qaherald` Go binary that drives full pherald ↔ Telegram round-trip automation with attached materials preserved under `docs/qa/<run-id>/`; final close-out is the v1.0.0-dev-0.0.1 (or canonical-named) release tag. Carry-over Wave 3c items (HRD-024 iherald paging, HRD-033 destructive-guard body, HRD-018..027 flavor bindings, the rest of the §43 command catalogue) remain queued. Each milestone followed by multi-mirror push + full anti-bluff verification. |
