...
- **e2e_bluff_hunt:** 60 PASS / 0 FAIL / 24 SKIP (Postgres-SASL carry-over RESOLVED this session).
...
- **Postgres-SASL carry-over (RESOLVED 2026-05-27)**: the e2e_bluff_hunt FAILs on Postgres-SASL invariants (E7-E12 + E14-E16 + E37-E42) are FIXED — `9b166c4` (auth env + PG password self-heal + Redis/tenant wiring + honest SKIP-with-reason) + `bdbe9f1` (Runner nil-Redis graceful PG-only fallback). e2e_bluff_hunt now reports 60 PASS / 0 FAIL / 24 SKIP.
