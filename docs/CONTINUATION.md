...
| HRD-122..130 | in_progress | GAP-3 §11.4.85 stress+chaos suite — IMPLEMENTED + e2e-green (`d2a60d3`); awaiting atomic Issues→Fixed migration. |
| HRD-131 | open | §11.4.93 SQLite workable-items SSoT migration Phase 1 (scope-lock filed); Phase 2+ pending. |
| HRD-132 | open | Runner idempotency-window bug — concurrent same-key replay duplicate-dispatches (2-4 of 1000× flood); fix = claim-before-dispatch + `runner.go:132` `CachedRcpt.WasReplay` race + HRD-125 `sends==1`. |
