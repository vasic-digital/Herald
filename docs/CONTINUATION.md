...
- **e2e_bluff_hunt:** invariants now run through **E96** (E89 Batch-A audit write-through + mode-flip; E90-E96 = Batch B flavor-bindings round-trips HRD-019..025; prior E81-E88 = GAP-3 §11.4.85 stress+chaos). Full e2e CONFIRMED this session = **79 PASS / 0 FAIL** (E1-E96 all green; E94/E95/E96 PASS verified post-commit `74649bb`; E3 -race green after the eventbus race fix `f5d1367`; E17 SKIP-with-reason on absent live-PG). Plus SKIP-with-reason for hardware/credential-absent invariants (live OOM, MTProto). Next free invariant id is **E97+**.
...
| Batch C (HRD-029..056) | next | **§43 command catalogue — IMMEDIATE NEXT UNIT.** Live CLI command bodies that PRODUCE the Subjects the Batch-B detectors classify (pherald commit-push, sherald destructive-guard, cherald creds-scan, rherald tag-mirror, bherald evidence-capture, …), grouped into 5 flavor clusters. Closing Batch C flips HRD-019..025 →Fixed. Next free e2e invariant id is **E97+**. Then Batch D (commons_infra HRD-081/085..090), Batch E (Wave 7 T6-T12 Slack HRD-115..121), then §11.4.93 SQLite (deferred). Plan `docs/superpowers/plans/2026-05-27-v1.0.0-execution.md`. |
