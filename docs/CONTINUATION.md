...
| Batch D (HRD-085..089) | in_progress (IMPLEMENTED 2026-05-28, `5899623`) | **commons_infra `pgxTaskRepository` — 16 of 17 methods implemented + REAL-PG verified.** GetByID/Update/Delete(soft) + UpdateStatus/Progress/Heartbeat/SaveCheckpoint + GetByStatus/GetPendingTasks/CountByStatus/GetTaskHistory + GetStaleTasks/GetByWorkerID + SaveResourceSnapshot/GetResourceSnapshots; migration `000013_task_resource_snapshots` (ledger 12→13). **14/14 integration round-trips PASS against live PG** (`docs/qa/HRD-085-089-20260528T070000Z/integration_realpg.log`); e2e E123 (gated `HERALD_BATCHD_LIVE=1` — self-boot collides with the e2e's shared PG). The real-PG run caught + fixed a latent `LogEvent` JSONB-as-[]byte→bytea 22P02 bug (now `string()`). **`MoveToDeadLetter` still `ErrUnsupported` → HRD-090** (dead-letter table + retention, separate effort). PG boot: `podman machine start` (VM was never-started) + the integration suite self-boots herald-postgres via QuickstartBoot; a stale `herald-e2e_/herald-quickstart_herald-pg` volume caused a one-time 28P01 auth fail — removed both, recreated fresh with `herald_dev`. |
| Wave 7 (HRD-110..121) | in_progress — CODE-COMPLETE 2026-05-28 | **Generic messenger framework + Slack — entire Wave 7 (T1-T12) landed.** Framework (HRD-110..114) Fixed pre-session: channels.Channel interface, init()-registered registry, per-channel inbox/<channel>/<sha256>.<ext>, generalized IsSelfEcho via SelfIdentity{Kind,Value}, multi-channel `pherald listen`. This session: **HRD-115** slack adapter Socket Mode (`54d4316`, vendored submodules/slack-go@v0.16.0/209ec09, 22 hermetic httptest tests), **HRD-116** qaherald raw-HTTP slack MessengerClient + channel-keyed Build (`6811114`, 30 tests, xoxb token NEVER in errors via sanitizeError + TestSlack_Send_ErrorDoesNotLeakToken), **HRD-117** spec V3 r12→r13 (`7f1b043`, §11.0 + §32.2 LIVE + §43), **HRD-118** e2e E124-E131 multi-channel invariants (`17ecd0b`, E127 LIVE-Slack triple-guarded against "no tests to run" PASS-bluff), **HRD-119** paired §1.1 mutation gate 4 PASS / 0 FAIL (`6737040`, 3 mutations: blank-IsSelfEcho + drop-Register + drop-thread_ts), **HRD-120** multi-channel §11.4.85 stress+chaos 12/12 PASS (`86dfa1e`, per-channel attribution + concurrency-under-fault), **HRD-121** docs/guides/MESSENGER_CHANNELS.md operator guide (`1037019`, 610 lines + 3 siblings). HRDs stay in_progress pending operator-live Slack evidence (mirrors Wave 6's v0.4.0 deferred-to-T13b convention). |
| v0.6.0 tag | operator-gated | Operator runs the live closed-loop against a real Slack workspace + commits `docs/qa/HRD-115-LIVE-<run-id>/` evidence per §107.x; then E127 converts SKIP→PASS automatically (its sc_anchor already cites the future live transcript path). Tag command: `git tag -a v0.6.0 -m "Wave 7 — multi-channel messenger framework"`. Required env for the live run: `HERALD_SLACK_BOT_TOKEN` (xoxb-) + `HERALD_SLACK_CHANNEL_ID` (Cxxx) + `HERALD_SLACK_APP_TOKEN` (xapp- for Socket Mode); operator-setup walkthrough at `docs/guides/MESSENGER_CHANNELS.md` §4 + §10 pre-deploy checklist. |
| RESUME-AT (next units) | next | (1) **Operator-live close-out wave** — flip HRD-019..025 (Batch B bindings) + HRD-029..056 §43 (Batch C) + HRD-085..089 (Batch D) + HRD-115..121 (Wave 7) →Fixed once their respective live workspaces / PG instances / messenger workspaces are exercised. Bundles with v0.4.0 (Wave 6 deferred) + v0.6.0 (Wave 7) tags. (2) **HRD-090** `MoveToDeadLetter` (dead-letter table + retention design) — the last `ErrUnsupported` TaskRepository method. (3) **Wave 8** — design new channels (Max / Email / Discord / WhatsApp / Viber) on the now-stable Wave 7 framework + the supervised-restart redesign noted in HRD-120's T11 chaos doc-comment. (4) HRD-008 quickstart validation, HRD-015 gate-I8, HRD-081 (upstream containers podman/docker detect). (5) §11.4.93 SQLite (HRD-131, operator-deferred). Plan `docs/superpowers/plans/2026-05-27-v1.0.0-execution.md` + `docs/superpowers/plans/2026-05-27-wave7-generic-messenger.md`. **HEAD=`1037019`**; Wave 7 e2e = 142 PASS / 0 FAIL (T9 invariants green, T10 mutation gate 4/0, T11 stress+chaos 12/12); inheritance gate 15/0 post-`.gitmodules` slack-go-submodule add. |
